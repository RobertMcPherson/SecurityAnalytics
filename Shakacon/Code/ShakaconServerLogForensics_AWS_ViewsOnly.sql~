--Make a directory on the Hadoop File System (HDFS) to store log files
hadoop dfs -mkdir data

--Copy data from Amazon S3 bucket, to Hadoop File System (HDFS) directory
hadoop dfs -cp 's3n://JunePresentation/Data/access*' /user/hadoop/data

--Start Hive
hive

--Add path for Hive Java archive file
add jar hive/contrib/hive-contrib-0.8.1.jar;

--View column headers in the Hive output
...
LOAD DATA INPATH "/user/hadoop/data/access*" INTO TABLE apachelog;
...
--View results
SELECT * FROM statusgroupings LIMIT 100;
...
--INSERT OVERWRITE LOCAL DIRECTORY '/mnt/hgfs/GitHub/SecurityAnalytics/Shakacon/RecommenderStatusData'
...
--INSERT OVERWRITE LOCAL DIRECTORY '/mnt/hgfs/GitHub/SecurityAnalytics/Shakacon/RecommenderStatus400and500Series'
...
--Note: useful to run on new data, but no instances of these in examples
SELECT * FROM apachelog
...
--INSERT OVERWRITE LOCAL DIRECTORY '/mnt/hgfs/GitHub/SecurityAnalytics/Shakacon/TopHostFailedLogongsByDay'
...
--INSERT OVERWRITE LOCAL DIRECTORY '/mnt/hgfs/GitHub/SecurityAnalytics/Shakacon/TopHostSuccessfulLogongsByDay'
...
--INSERT OVERWRITE LOCAL DIRECTORY '/mnt/hgfs/GitHub/SecurityAnalytics/Shakacon/TopHostFailedLogonsByMonth'
...
--INSERT OVERWRITE LOCAL DIRECTORY '/mnt/hgfs/GitHub/SecurityAnalytics/Shakacon/TopHostSuccessfulLogonsByMonth'
...
--Show ratio of failed to successful queries by month
SELECT a.yearmonth
...
--INSERT OVERWRITE LOCAL DIRECTORY '/mnt/hgfs/GitHub/SecurityAnalytics/Shakacon/FailedRequestsByDay'
...
--INSERT OVERWRITE LOCAL DIRECTORY '/mnt/hgfs/GitHub/SecurityAnalytics/Shakacon/UnstackedStatusCodes'
...
--Write the above view results to a file in the local "home" directory - not the Hadoop Directory File System (HDFS) 
INSERT OVERWRITE LOCAL DIRECTORY 'SummedStatusByDay'
...
--Same table as above, but filter to include only failed status codes of '500' series
DROP TABLE request_field_words;
CREATE TABLE request_field_words (NEW_ITEM ARRAY<STRUCT<ngram:array<string>, estfrequency:double>>);
INSERT OVERWRITE TABLE request_field_words
SELECT ngrams(sentences(lower(request)), 1, 100) as word_map FROM apachelog WHERE status IN ('500');

SELECT * FROM request_field_words LIMIT 20;
...
SELECT * FROM request_field LIMIT 20;

--Create Failed Server Requests Table for Potential External Analysis--
...
INSERT OVERWRITE LOCAL DIRECTORY '/mnt/hgfs/GitHub/SecurityAnalytics/Shakacon/RequestsFailed'
